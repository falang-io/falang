...
/**
 * 
 * @param param0     const relativePathArray = await this.project.getSchemeRelativePathArray(this.path);
...
export const getFullTypeName = async ({
  ...
  switch (type.type) {
    case 'struct':
      ...
      const relativePathArray = await project.getSchemeRelativePathArray(structItem.path);
      const namespaceName = `Falang_${relativePathArray.join('_')}`;
      return `${namespaceName}::${structItem.name}`;
    case 'array':
      ...
      return `std::vector<${fullElementTypeName}>`;
    case 'boolean':
      return 'bool';
    case 'number':
