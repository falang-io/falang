...
export interface IGenerateExpressionParams {
  ...
  schemeBuilder: CppLogicSchemeBuilder,
  context?: TContext,
}
...
const generateExpressionPrivate = async ({
  ...
  if (math.isOperatorNode(node)) {
    switch (node.fn) {
      case 'pow':
        {
          //schemeBuilder.addImport('#include <math.h>');
...
const writeFunctionNode = async ({
  ...
  switch (fnName) {
    case 'random':
      schemeBuilder.addImport('#include <stdlib.h>');
      return convertSymbol({
        expression: '(float)std::rand() / (float)RAND_MAX',
