...
import { ILogicSchemeBuilderParams, LogicSchemeBuilder } from "../LogicSchemeBuilder";
...
//ILogicSchemeBuilderParams
export interface IJavascriptLogicSchemeBuilderParams extends ILogicSchemeBuilderParams {
  singleFile?: boolean
}

export class JavascriptLogicSchemeBuilder extends LogicSchemeBuilder {
  readonly singleFile?: boolean
  private readonly imports = new Set<string>();

  constructor(params: IJavascriptLogicSchemeBuilderParams) {
    super(params);
    this.singleFile = params.singleFile;
  }
  ...
  async importFunction(item: IAvailableFunctionItem) {
    if (!this.singleFile) {
      ...
      const importText = `const { ${item.name} } = require('${relativeFilename}');`;
      this.addImport(importText);
    } else {
      /*const pathArray = await this.project.getSchemeRelativePathArray(item.path);
      this.addImport(`const { ${item.name} } = _falangRegistry['${pathArray.join('/')}'];`);*/
    }
  }
