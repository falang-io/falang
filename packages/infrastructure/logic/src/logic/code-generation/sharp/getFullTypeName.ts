...
export const getFullTypeName = async ({
  ...
  switch (type.type) {
    case 'struct':
      ...
      const pathArray = await project.getSchemeRelativePathArray(structItem.path);
      return `Falang.${pathArray.join('.')}.${structItem.name}`;
    case 'array':
      ...
      return `System.Collections.Generic.List<${fullElementTypeName}>`;
    case 'boolean':
      return 'bool';
    case 'number':
