...
export const getFullTypeNamePrivate = async (project: LogicProjectStore, path: string, name: string): Promise<string> => {
  const relativePathArray = await project.getSchemeRelativePathArray(path)
  return `crate::falang::${relativePathArray.join('::')}::${name}`;
}
...
export const getFullTypeName = async ({
  ...
  switch (type.type) {
    case 'struct':
      ...
      if (!structItem) throw new Error(`Internal: struct item not found ${type.schemeId}/${type.iconId}`);
      return getFullTypeNamePrivate(project, structItem.path, structItem.name)
    case 'array':
      ...
      return `alloc::vec::Vec<${fullElementTypeName}>`;
    case 'boolean':
      return 'bool';
    case 'number':
    ...
    case 'enum': {
      ...
      return getFullTypeNamePrivate(project, enumItem.path, enumItem.name)
    };
  }
