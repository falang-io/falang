...
export class RustLogicSchemeBuilder extends LogicSchemeBuilder {
  ...
  async build(): Promise<string> {
    if(this.scheme.documentType === 'logic_external_apis') return '';
  ...
  async generateEmptyValue(type: TTypeInfo): Promise<string> {
    switch (type.type) {
      ...
      case 'boolean':
        return 'false';
      case 'struct': 
        return `${await this.getFullTypeName(type)}::new()`;
      case 'array':
        return `vec![]`;
      default:
  ...
  async getFileNameSpace(path: string) {
    const relativePath = await this.fs.relativePath(await this.getSchemesRootPath(), path);
    const pathArray = relativePath.replace('.falang.json', '').split(/[\/\\]/).filter(item => item !== '.');
    const importText = `crate::falang::${pathArray.join('::')}`;
    return importText;
  }
