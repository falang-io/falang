...
type Props = React.HTMLAttributes<HTMLDivElement> & {
  ...
  getAutoCompleteOptions?: (code: string, position: number) => AutocompleteOption[];
...
export class BaseCodeEditor extends React.Component<Props, State> {
  static defaultProps = {
    tabSize: 2,
    insertSpaces: true,
    ignoreTabKey: false,
  ...
  private _handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'Tab' && !ignoreTabKey && this.state.capture) {
      ...
      if (selectionStart === selectionEnd) {
        if (this.state.autoCompleteOptions.length && !e.ctrlKey && !e.shiftKey) {
          const current = this.state.autoCompleteOptions[this.state.autoCompleteSelectedIndex];
          if (current) {
            e.preventDefault();
            const updatedSelection = selectionStart + current.toInsert.length;
            this._applyEdits({
      ...
      let chars: [string, string] | null = null;
    ...
    } else if (e.key === 'ArrowDown' && this.state.autoCompleteOptions.length) {
      e.preventDefault();
      this.setState((state) => ({
        ...state,
        autoCompleteSelectedIndex: Math.min(state.autoCompleteSelectedIndex + 1, state.autoCompleteOptions.length - 1),
      }))
    } else if (e.key === 'ArrowUp' && this.state.autoCompleteOptions.length) {
      e.preventDefault();
      this.setState((state) => ({
        ...state,
        autoCompleteSelectedIndex: Math.max(state.autoCompleteSelectedIndex - 1, 0),
      }))
    }
  };
  ...
  private _handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (selectionStart === selectionEnd && this.props.getAutoCompleteOptions) {
      newOptions = this.props.getAutoCompleteOptions(value, selectionStart);
    }
  ...
  render() {
    ...
    return (
      <div {...rest} style={{ ...styles.container, ...style }}>
        ...
        {this.state.autoCompleteOptions.length ? <AutoCompleteDiv>
          {this.state.autoCompleteOptions.map((o, index) =>
            <AutoCompleteItemDiv
              key={index} 
              style={{ fontWeight: index === this.state.autoCompleteSelectedIndex ? 'bold' : 'normal' }}
              onClick={(e) => {
                const current = this.state.autoCompleteOptions[index];
                if(!this._input) return;
                const { value, selectionStart, selectionEnd } = this._input;
                if (current) {
                  e.preventDefault();
                  const updatedSelection = selectionStart + current.toInsert.length;
                  this._applyEdits({
