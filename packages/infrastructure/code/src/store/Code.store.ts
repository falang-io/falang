...
export const codeStyle = schemeBaseTextStyle;
...
export interface CodeStoreParams {
  scheme: SchemeStore,
  code: string,
  language: string
  minHeight?: number
...
export class CodeStore {
  ...
  @observable width: TComputedProperty<number> = 0;
  readonly language: string;
  readonly minHeight: number;
  readonly singleLine: boolean;

  constructor(params: CodeStoreParams) {
  ...
  @computed get renderCode(): boolean {
  ...
  @computed get computedLines(): ITokenLineWord[][] {
    ...
    const width = Math.max(getComputedValue(this.width, 0) - TEXT_PADDING_WIDTH * 2, 0);
